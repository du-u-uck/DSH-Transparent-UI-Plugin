...
export function apply(ctx: ClientContext): void {
  ...
  // New DSH slots API (0.1.1-rc.2): `settings.plugin.item` is now a KEYED slot
  // (was list), so the registration uses `key` (the settings namespace) instead
  // of `id`/`order`, and `ctx.slots.inject` was removed — register directly
  // under an effect disposer.
  ...
  }, AquaPluginCard), 'ui-aqua: settings.plugin.item')

  // Glass knobs row in the General section, directly under Appearance (10).
  // New DSH slots API (0.1.1-rc.2): register directly under an effect.
  ctx.effect(() => ctx.slots.register({
  ...
  }, AquaAppearanceRow), 'ui-aqua: settings.general.item')
}
